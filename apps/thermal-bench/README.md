...
which. X steps both load dials together, Square steps the CPU dial alone, Triangle steps
...
cap removed. If either temperature reaches 88 °C — **or the console refuses to report a
...
ghost lines in the same colours, aligned on elapsed time — so the gap between the solid and
