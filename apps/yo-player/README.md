...
`/dev_hdd0/tmp/yo-player/` (history, subscriptions, Watch Later, downloads).
...
- **R3** (right stick click) — **download** the highlighted video: its video and audio are combined into one
  `.mkv` under `/dev_hdd0/tmp/yo-player/downloads`, named after the video. Downloads run in the background
...
History, subscriptions, Watch Later, and downloads all live as plain files under `/dev_hdd0/tmp/yo-player/`
