// zip - real deflate compression/decompression and ZIP archive reading/writing, vendored
// from miniz 3.1.2 (public domain / Unlicense, see below), plus our own VFS-bridging wrapper
// API at the bottom of this file and zip.c (zipPathsProgress/unzipArchiveProgress/...).
//
// trimmed from the original: the zlib-compatible wrapper layer (mz_deflate/mz_inflate,
// mz_compress2/mz_uncompress, the z_stream-style API), the PNG writer
// (tdefl_write_image_to_png_file_in_memory[_ex]), and all stdio/FILE*-based archive
// open/init/extract variants have been removed or compiled out (MINIZ_NO_STDIO,
// MINIZ_NO_TIME, MINIZ_NO_ZLIB_APIS are forced on below) - none of it is reachable from
// our wrapper, and this codebase's linker can't dead-strip unused code, so leaving it in
// would bloat every binary that links this lib.
//
// the ~10 public functions our wrapper calls have been renamed to house style everywhere
// they appear (declaration here, definition + internal callers in zip.c); everything else
// (internal statics, the low-level tdefl_/tinfl_ compressor, unused reader/writer variants,
// types/typedefs/constants) keeps its original miniz name:
//   mz_zip_reader_init                 -> initZipReader
//   mz_zip_reader_end                  -> closeZipReader
//   mz_zip_reader_get_num_files        -> getZipFileCount
//   mz_zip_reader_file_stat            -> getZipFileStat
//   mz_zip_reader_is_file_a_directory  -> isZipFileDirectory
//   mz_zip_reader_extract_to_callback  -> extractZipFileToCallback
//   mz_zip_writer_init_v2              -> initZipWriter
//   mz_zip_writer_add_read_buf_callback -> addZipEntryFromCallback
//   mz_zip_writer_finalize_archive     -> finalizeZipWriter
//   mz_zip_writer_end                  -> closeZipWriter
//   mz_zip_writer_add_mem               -> addZipEntryFromMemory
...
/* miniz.c 3.1.2 - public domain deflate/inflate, zlib-subset, ZIP reading/writing/appending, PNG writing
   ...
   * ZIP archive API notes:
     ...
     Alternately, you can iterate through all the files in an archive (using getZipFileCount()) and
     retrieve detailed info on each file by calling getZipFileStat().
...
extern "C"
{
    ...
    MINIZ_EXPORT mz_bool initZipReader(mz_zip_archive *pZip, mz_uint64 size, mz_uint flags);
    ...
    /* The FILE will NOT be closed when closeZipReader() is called. */
    ...
    MINIZ_EXPORT mz_bool closeZipReader(mz_zip_archive *pZip);
    ...
    MINIZ_EXPORT mz_uint getZipFileCount(mz_zip_archive *pZip);
    ...
    MINIZ_EXPORT mz_bool isZipFileDirectory(mz_zip_archive *pZip, mz_uint file_index);
    ...
    MINIZ_EXPORT mz_bool getZipFileStat(mz_zip_archive *pZip, mz_uint file_index, mz_zip_archive_file_stat *pStat);
    ...
    MINIZ_EXPORT mz_bool extractZipFileToCallback(mz_zip_archive *pZip, mz_uint file_index, mz_file_write_func pCallback, void *pOpaque, mz_uint flags);
...
/* TODO */
    ...
    /* Universal end function - calls either closeZipReader() or closeZipWriter(). */
    ...
    /*Set pZip->m_pWrite (and pZip->m_pIO_opaque) before calling mz_zip_writer_init or initZipWriter*/
    /*The output is streamable, i.e. file_ofs in mz_file_write_func always increases only by n*/
    MINIZ_EXPORT mz_bool mz_zip_writer_init(mz_zip_archive *pZip, mz_uint64 existing_size);
    MINIZ_EXPORT mz_bool initZipWriter(mz_zip_archive *pZip, mz_uint64 existing_size, mz_uint flags);
    ...
    /* For archives opened using mz_zip_reader_init_file, pFilename must be the archive's filename so it can be reopened for writing. If the file can't be reopened, closeZipReader() will be called. */
    /* For archives opened using mz_zip_reader_init_mem, the memory block must be growable using the realloc callback (which defaults to realloc unless you've overridden it). */
    /* Finally, for archives opened using initZipReader, the mz_zip_archive's user provided m_pWrite function cannot be NULL. */
    ...
    MINIZ_EXPORT mz_bool addZipEntryFromMemory(mz_zip_archive *pZip, const char *pArchive_name, const void *pBuf, size_t buf_size, mz_uint level_and_flags);

    /* Like addZipEntryFromMemory(), except you can specify a file comment field, and optionally supply the function with already compressed data. */
    ...
    MINIZ_EXPORT mz_bool addZipEntryFromCallback(mz_zip_archive *pZip, const char *pArchive_name, mz_file_read_func read_callback, void *callback_opaque, mz_uint64 max_size,
    ...
    /* After an archive is finalized, the only valid call on the mz_zip_archive struct is closeZipWriter(). */
    /* An archive must be manually finalized by calling this function for it to be valid. */
    MINIZ_EXPORT mz_bool finalizeZipWriter(mz_zip_archive *pZip);
    ...
    MINIZ_EXPORT mz_bool closeZipWriter(mz_zip_archive *pZip);
...
// section: our own VFS-bridging wrapper (implemented at the end of zip.c)
//
// cross-backend tree operation alongside copyTreeProgress/deleteTreeProgress in vfs.h -
// zip/unzip is generically useful VFS-path work, not app-specific.
#include <stdint.h>

// zips paths (files or directories, walked recursively) into one archive at destZipPath.
// each top-level path's basename becomes its root entry name inside the zip. onBytes/cancelled
// follow the copyTreeProgress convention. 0 ok, -1 error, 1 cancelled.
int zipPathsProgress(const char **paths, int count, const char *destZipPath,
                     void (*onBytes)(uint64_t), int (*cancelled)(void));

// sum of uncompressed bytes across paths, for the progress total (mirrors measureTree).
uint64_t measureZipSource(const char **paths, int count, int (*cancelled)(void));

// extracts srcZipPath into destDir (created if absent), merging like mergeTreeProgress:
// replaceExisting governs collisions with files already in destDir. entry names are rejected
// if they'd escape destDir (absolute path, or a ".." segment) - an archive's contents are
// untrusted input. 0 ok, -1 error, 1 cancelled.
int unzipArchiveProgress(const char *srcZipPath, const char *destDir, int replaceExisting,
                         void (*onBytes)(uint64_t), int (*cancelled)(void));

// sum of uncompressed entry sizes in srcZipPath; 0 if the archive can't be opened.
